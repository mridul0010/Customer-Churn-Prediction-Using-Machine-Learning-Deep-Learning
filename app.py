...
def load_dataset():
    try:
        ...
        st.error("❌ Error: 'train.csv' appears to be empty or corrupted.")
        st.stop()
    except Exception as e:
        st.error(f"❌ Error loading train.csv: {str(e)}")
        st.stop()
...
def validate_required_files():
    ...
    required_files = ["train.csv", "pipeline.pkl"]
    ...
    if missing_files:
        ...
        **Please ensure these files are in the same directory as the application:**
        - `train.csv` - Training dataset for analytics and model evaluation
        - `pipeline.pkl` - Trained XGBoost model pipeline
        
        **Current working directory:** {os.getcwd()}
        """)
        st.stop()

# Validate files on app startup
